...
func TestIntegration_MCPHealthEndpoint(t *testing.T) {
	...
	srv, err := server.New(client, nil, nil, nil, nil, nil, nil, "test-token-health", slog.Default())
	if err != nil {
...
func TestIntegration_MCPAuthRejected(t *testing.T) {
	...
	srv, err := server.New(client, nil, nil, nil, nil, nil, nil, "test-token-auth", slog.Default())
	if err != nil {
		t.Fatalf("server.New: %v", err)
	}
...
func TestIntegration_MCPAuthWithBadToken(t *testing.T) {
	...
	srv, err := server.New(client, nil, nil, nil, nil, nil, nil, "test-token-bad", slog.Default())
	if err != nil {
		t.Fatalf("server.New: %v", err)
	}
...
func TestIntegration_MCPAuthAccepted(t *testing.T) {
	...
	srv, err := server.New(client, nil, nil, nil, nil, nil, nil, token, slog.Default())
	if err != nil {
		t.Fatalf("server.New: %v", err)
	}
